...
export type Recording = {
  /** The word the run reached — confirmed if `confirmed`, its best guess
   * otherwise. */
  answer: string;
  /** False when the search never reached 100%: `answer` is its best guess,
   * not a verified one. Absent on old recordings, which were always solves. */
  confirmed?: boolean;
  recorded_at: string;
  events: Message[];
};
